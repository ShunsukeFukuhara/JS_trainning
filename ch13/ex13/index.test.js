...
const basePath = "./ch13/ex13/test/"; // テストファイルのベースパス

describe("walk", () => {
  test("対象のファイルやディレクトリが全て網羅されている", async () => {
    const result = [];
  ...
  test("存在しないファイルやディレクトリが入っていない", async () => {
    const result = [];
    for await (const entry of walk(basePath)) {
      result.push({
  ...
  test("存在しないディレクトリを指定した場合", async () => {
    await expect(async () => {
      const result = [];
      for await (const entry of walk("./nonexistent")) {
        result.push(entry);
      }
    }).rejects.toThrow();
  });
